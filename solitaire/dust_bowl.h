...
void tickDustBowl(Game* game) {
	...
	float xpos = game->dustBowlSpriteClass.Buffer_vertexOffsetData[0];
	float ypos = game->dustBowlSpriteClass.Buffer_vertexOffsetData[1];
	...
	if (game->mouseX > xpos && game->mouseX < xpos + DEFUALT_MODEL_SCALE && game->mouseY > ypos && game->mouseY < ypos + DEFUALT_MODEL_SCALE) {
		if (!dustBowl.mouseHovering) {
			...
			IndexReference reference = queueScaleAnimation(game->dustBowlSpriteClass, 0, 1.0f, 1.5f, 5, game->gameTime);
